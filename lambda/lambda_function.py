...
def lambda_handler(event, context):
    ...
    try:
        if not media_file_id or not target_language or not translation_job_id or not user_id:
            print(F"Media file ID or target language or translation job ID or user ID is missing")
            return
        ...
        if not translation_job:
            print(F"Translation job not found")
            return

        print(f"Processing translation job with id: {translation_job_id} and target language: {target_language}")
